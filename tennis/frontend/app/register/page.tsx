...
export default function RegisterPage() {
    ...
    const handleAuth = (data: { access_token: string; user_id: string; email: string; name: string; subscription_tier: string }) => {
    ...
    const handleGoogleCallback = async (response: { credential: string }) => {
        ...
        try {
            ...
            const axiosErr = err as { response?: { data?: { detail?: string } } };
            setError(axiosErr.response?.data?.detail ?? 'Google sign-up failed.');
        ...
        <div className="min-h-[calc(100vh-4rem)] flex items-center justify-center px-4">
            <div className="w-full max-w-sm">
                ...
                <div className="card">
                    ...
                    <p className="text-white/40 text-sm mb-6">Free · 3 uploads per month</p>
                    ...
                    <p className="text-center text-sm text-white/40 mt-5">
                        ...
                        <Link href="/login" className="text-brand-400 hover:text-brand-300 underline underline-offset-2">
                            Sign in
                        </Link>
                    </p>
                </div>
                ...
                <div className="card mt-4 space-y-3">
                    <p className="text-xs text-white/40 uppercase tracking-wider font-semibold">Free plan includes</p>
                    {['3 uploads per month', 'Basic shot & rally stats', 'Line call detection'].map((f) => (
                        <div key={f} className="flex items-center gap-2 text-xs text-white/60">
                            <div className="w-1.5 h-1.5 rounded-full bg-court-400" />
                            {f}
                        </div>
                    ))}
                </div>
